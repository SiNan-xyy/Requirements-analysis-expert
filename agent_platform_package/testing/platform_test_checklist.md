...
- RAG 可以检索 12 份上传材料，包括影刀能力中文卡、腾讯文档/在线表格能力、报表采集场景、分支异常确认规则、HTML 中文词典、报告质量规则和验证码能力边界。
...
- 统一 HTML 报告必须来自同一份结构化事实源；兼容的对客 HTML 和实施对齐 HTML 不得创造新事实。
- 统一 HTML 报告是需求落地分析和搭建对齐包，不是最终构建指南。
- 统一 HTML 报告应中文优先，不应直接暴露大量英文枚举、机器字段或 IT 技术术语。
- 统一 HTML 报告的每个关键流程步骤应展示可参考的影刀能力或说明暂无明确能力依据。
